...
module.exports = app => {
  ...
  const signIn = async (req, res) => {
    ...
    try {
      ...
      if (user.deleted)
        throw "Sua conta esta suspensa, em caso de reinvidicação entre em contato com o administrador do sistema";
    ...
    try {
      ...
      if (exist && (exist._id || exist.id)) {
        if (user.email !== exist.email)
          throw "Acesso não autorizado, seu e-mail de acesso foi alterado.";
